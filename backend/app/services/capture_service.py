...
Os passos 4-6 rodam CONCORRENTEMENTE entre frames (limitados por um
semáforo) em vez de sequencialmente: cada frame aprovado na nitidez faz até
duas chamadas de rede ao Roboflow (validação de cocho completo + upload), e
rodar isso frame a frame em série para ~20-30 candidatos por vídeo facilmente
ultrapassava o timeout de upload do app móvel.
"""
...
from app.services.frame_extractor import ExtractedFrame, encode_jpeg, extract_frames
...
# Limite de chamadas de rede concorrentes ao Roboflow por captura. Alto o
# bastante para acelerar bastante o processamento, baixo o bastante para não
# estourar rate limit do Roboflow nem sobrecarregar o plano gratuito do
# Render (CPU/rede compartilhados e limitados).
MAX_CONCURRENT_FRAME_REQUESTS = 6
...
class CaptureService:
    ...
    async def process_capture(
        ...
        *,
        ...
        try:
            ...
            # --- 4-6. Nitidez + validação de cocho completo + upload,
            # concorrente entre frames (limitado pelo semáforo) ---
            semaphore = asyncio.Semaphore(MAX_CONCURRENT_FRAME_REQUESTS)
            ...
            # asyncio.gather preserva a ordem de entrada, mas deixamos
            # explícito por robustez caso a extração não venha ordenada.
            frame_results = sorted(frame_results, key=lambda r: r.frame_index)

            aprovados = sum(1 for r in frame_results if r.status == FrameStatus.aprovado)
            desfocados = sum(1 for r in frame_results if r.status == FrameStatus.rejeitado_desfoque)
            cocho_incompleto = sum(
                1 for r in frame_results if r.status == FrameStatus.rejeitado_cocho_incompleto
            )
            falhas_upload = sum(1 for r in frame_results if r.status == FrameStatus.falha_upload)
    ...
    async def _process_frame(
        ...
        *,
        ...
        """Processa um único frame: nitidez -> validação de cocho -> upload.

        As duas chamadas de rede ao Roboflow (validação e upload) ficam
        atrás do semáforo compartilhado entre todos os frames da captura,
        para limitar a concorrência total sem serializar o processamento.
        """
